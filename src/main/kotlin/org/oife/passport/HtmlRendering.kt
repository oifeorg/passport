...
private fun String.markDownParser(flavour: GFMFlavourDescriptor): ASTNode = MarkdownParser(flavour).parse(IElementType("ROOT"), this)
...
private fun SinglePassport.toPlaceholderMap(meta: PassportMeta): Map<String, String> =
    mapOf(
...
private fun CombinedPassport.toPlaceholderMap(): Map<String, String> =
    mapOf(
        Placeholder.PASSPORT_INDEX_ITEMS to
            renderIndexItems(
                configs = passportConfigs,
                indexTemplate = indexTemplate,
            ),
        Placeholder.PASSPORT_ARTICLE_ITEMS to
...
fun SinglePassport.renderHtml(meta: PassportMeta): String =
    htmlTemplate.replacePlaceholders(
        toPlaceholderMap(meta),
    )

fun CombinedPassport.renderHtml(): String =
    htmlTemplate.replacePlaceholders(
        toPlaceholderMap(),
    )
...
fun renderFontStyles(passportConfigs: List<PassportMeta>): String =
    buildString {
        passportConfigs
            .distinctBy { it.font }
...
private fun PassportMeta.toIndexPlaceholders(indexTemplate: String): String =
